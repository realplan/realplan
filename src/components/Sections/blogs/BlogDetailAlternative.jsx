...
export default function AlternativeSolutions({ blogs = [] }) {
  return (
    <section className="px-[clamp(1rem,2vw+0.5rem,4rem)] xl:px-[6.2rem] 2xl:px-[9rem] py-[clamp(3rem,5vw,6rem)]">
      ...
      <div className="mt-[clamp(2rem,3vw,3rem)] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {blogs.slice(0, 3).map((item, i) => (
          ...
          <Link key={i} href={`/blogs/${item.slug}`} className="block">
            <div className="group relative rounded-2xl overflow-hidden flex flex-col h-full transition-all duration-300 hover:scale-[1.02] hover:shadow-xl cursor-pointer">
              ...
              <div className="relative z-10 bg-[#EDEDED] p-4 sm:p-5 flex flex-col h-full rounded-2xl group-hover:bg-transparent transition-colors duration-300">
                ...
                {/* Title */}
                <h3 className="text-[#FF8205] text-[clamp(1.2rem,1vw+0.8rem,1.5rem)] font-normal leading-snug group-hover:text-black transition-colors duration-300">
                  {item.title}
                </h3>
