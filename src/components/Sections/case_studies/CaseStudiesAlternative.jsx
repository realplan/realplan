...
import { CASE_STUDIES_ALTERNATIVE_CONTENT } from "@/data/caseStudiesAlternativeData";
import { CASE_STUDIES } from "@/data/caseStudiesData";
import { useEffect, useRef } from "react";

const getAlternativeCards = (selectedCategory) => {
  const normalizedSelectedCategory = selectedCategory?.trim().toLowerCase();

  const selectedIndex = CASE_STUDIES.findIndex(
    (item) =>
      item.category.trim().toLowerCase() === normalizedSelectedCategory
  );

  const startIndex = selectedIndex >= 0 ? selectedIndex + 1 : 1;

  return Array.from({ length: 3 }, (_, offset) => {
    const caseStudy =
      CASE_STUDIES[(startIndex + offset) % CASE_STUDIES.length] ??
      CASE_STUDIES[offset];

    const alternativeContent = CASE_STUDIES_ALTERNATIVE_CONTENT.find(
      (item) =>
        item.category.trim().toLowerCase() ===
        caseStudy.category.trim().toLowerCase()
    );

    return {
      image: alternativeContent?.image,
      title: alternativeContent?.category ?? caseStudy.category,
      category: caseStudy.category,
      description:
        alternativeContent?.description ??
...
export default function AlternativeSolutions({
  ...
  // ✅ Scroll AFTER category updates (no flicker)
  useEffect(() => {
  ...
  const alternativeCards = getAlternativeCards(selectedCategory);

  const activeAlternativeContent =
    CASE_STUDIES_ALTERNATIVE_CONTENT.find(
      (item) =>
        item.category.trim().toLowerCase() === normalizedSelectedCategory
    ) ?? CASE_STUDIES_ALTERNATIVE_CONTENT[0];
  ...
  return (
    <section className="px-[clamp(1rem,2vw+0.5rem,4rem)] xl:px-[6.2rem] 2xl:px-[9rem] py-[clamp(3rem,5vw,6rem)]">
      ...
      <div className="mt-[clamp(2rem,3vw,3rem)] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {alternativeCards.map((item, i) => (
          <div
            key={item.title}
            onClick={() => onCategorySelect?.(item.category)} // ✅ FIXED
            className="group cursor-pointer relative rounded-2xl overflow-hidden flex flex-col h-full transition-all duration-300 hover:scale-[1.02] hover:shadow-xl"
          >
            {/* Gradient overlay */}
